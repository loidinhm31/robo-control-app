...
import type { SystemMetrics } from "@robo-fleet/shared/types";
import { Socket } from "socket.io-client";
import { StatPanel } from "../molecules";
import { getMetricColor } from "@robo-fleet/ui/hooks";
...
export const FloatingMetrics: React.FC<FloatingMetricsProps> = ({
  ...
  const getMetricBarColor = (value: number, type: TabType): string => {
    switch (type) {
      ...
      case "memory":
        ...
        if (value < 500) return "bg-warning";
        return "bg-error";
      case "latency":
        if (value < 50) return "bg-success";
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="group relative flex items-center gap-2.5 px-4 py-3 bg-primary text-white rounded-full shadow-2xl hover:shadow-primary/50 hover:brightness-110 transition-all duration-300 hover:scale-105 cursor-pointer"
        >
  ...
  const systemStats = currentMetrics ? [
    ...
    { label: "Memory", value: currentMetrics.total_memory_mb, decimals: 0, unit: "MB", color: "text-info" },
    { label: "Latency", value: currentMetrics.end_to_end_latency_ms, decimals: 1, unit: "ms", color: "text-primary" },
  ] : [];
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-[420px]">
      <div className="glass-card rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-3 border-b border-white/10 bg-primary/20">
