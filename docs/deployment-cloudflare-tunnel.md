...
- Cloudflare Tunnel already running for the main domain (e.g. `glean-oak-v001.cloud`)
...
```bash
cd /path/to/glean-oak-sync/glean-oak-server
./scripts/setup-cloudflare-tunnel.sh glean-oak "" "" robo-fleet.glean-oak-v001.cloud --patch
```

The script:
1. Routes DNS `robo-fleet.glean-oak-v001.cloud` → `<tunnel-id>.cfargotunnel.com` via `cloudflared tunnel route dns`
...
# Route DNS
cloudflared tunnel route dns <TUNNEL_NAME> robo-fleet.glean-oak-v001.cloud
```
...
```yaml
  ...
  - hostname: robo-fleet.glean-oak-v001.cloud
...
curl -v "https://robo-fleet.glean-oak-v001.cloud/socket.io/?EIO=4&transport=polling"

# WebSocket transport (requires wscat: npm i -g wscat)
wscat -c "wss://robo-fleet.glean-oak-v001.cloud/socket.io/?EIO=4&transport=websocket"
```
...
```env
VITE_SOCKET_IO_URL=https://robo-fleet.glean-oak-v001.cloud
```
...
The browser's `Origin` header is set to the **page** origin (e.g. `https://glean-oak-v001.cloud`), not the WebSocket target. Ensure orchestra's `ALLOWED_ORIGINS` includes the page origin:

```yaml
# robo-fleet-dora-rs/docker/docker-compose.yml
ALLOWED_ORIGINS: "${ALLOWED_ORIGINS:-https://app.glean-oak-v001.cloud,http://localhost:1420,http://localhost:3000,http://localhost:5173}"
```
...
If the domain changes from `glean-oak-v001.cloud`:
